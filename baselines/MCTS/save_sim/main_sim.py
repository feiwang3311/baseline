...
from models import model, model3, model2, load, save
import scipy.sparse as sp
from sl_buffer_sim import slBuffer_allFile
from mct_sim import MCT
...
def self_play(args, built_model, status_track):
    ...
    with tf.Session() as sess:
        ...
        # prepare the supervised learning buffer to write to
        ...
        for i in status_track.get_nbatch_index(args.nbatch, args.n_train_files):
            MCTList.append(MCT(args.train_path, i, args.max_clause, args.max_var, args.nrepeat, sl_Buffer, 
                ...
                for i in range(args.nbatch):
                    if needMore[i]:
                        ...
                        while temp is None:
                            ...
                            if all_files_done:
                                break
                            MCTList[i] = MCT(sat_path, next_file_index, args.max_clause, args.max_var, 1, sl_Buffer = None, tau = lambda x: 0.001, resign = 400)
                            next_file_index += 1
...
def ev_ss(args, built_model, status_track, file_no):
    ...
    with tf.Session() as sess:
        ...
        MCT58 = MCT(sat_path, file_no, args.max_clause, args.max_var, 1, sl_Buffer = None, tau = lambda x: 0.9, resign = 80)
...
def main():
    ...
    parser.add_argument('--train_path', type = str, help='where are training files', default="satProb/uf20_train32/")
    parser.add_argument('--test_path', type = str, help='where are test files', default="satProb/uf20_cv200/")
    ...
    parser.add_argument('--n_cycles', type = int, help="how many cycles of self_play -> super_train -> model_ev do we want to run", default = 1)
